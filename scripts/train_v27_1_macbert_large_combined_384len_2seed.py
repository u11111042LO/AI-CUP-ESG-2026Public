...
# -*- coding: utf-8 -*-
"""
AI CUP VeriPromiseESG - v27_1 MacBERT-large combined 384len holdout 2seed
...
4. 儲存 base_seed_*.pt、quality_seed_*.pt、27-1SumForGpt.txt/json，並同步保留 run_summary_for_chatgpt.txt/json。
5. 之後可用同一支腳本載入模型產生 Sub_v27_1.csv；submission 預設輸出 N/A，避免 AIdea 格式錯誤。

建議放置：
/content/gdrive/MyDrive/aicup_esg_2026/Scripts/train_v27_1_macbert_large_combined_384len_2seed.py

Colab 執行範例：
!python /content/gdrive/MyDrive/aicup_esg_2026/Scripts/train_v27_1_macbert_large_combined_384len_2seed.py
...
RUN_NAME = "v27_1_macbert_large_combined_384len_2seed"
...
DEFAULT_OUTPUT_DIR = "/content/gdrive/MyDrive/aicup_esg_2026/outputs/outputs_esg_v27_1_macbert_large_combined_384len_2seed"
# v16_2 是超細搜版本，預設沿用 v16-1 輸出中的已訓練權重，避免重訓。
DEFAULT_SOURCE_MODEL_DIR = ""  # v27-1 預設不複製舊模型，改用另一個 holdout split 從頭訓練，增加模型多樣性

# v27-1：把 train + official val 合併後，改用另一個 holdout seed 留 10% 當內部 holdout。
...
def run_predict(args, tokenizer, device):
    ...
    summary_path = os.path.join(output_dir, "27-1SumForGpt.json")
    ...
    out_path = args.submission_path or os.path.join(output_dir, "Sub_v27_1.csv")
...
def main():
    ...
    if args.mode in ["train", "eval", "train_eval_predict"]:
        ...
        summary = {
            ...
            "score_note": "selected_validation_score is internal holdout after combining train+official_val; not directly comparable with official-val runs. v27-1 uses MacBERT-large with the same combined holdout setting as v26-1 for bold model-family diversity.",
        ...
        summary_json_path = os.path.join(args.output_dir, "27-1SumForGpt.json")
        summary_txt_path = os.path.join(args.output_dir, "27-1SumForGpt.txt")
